...
| 用户说 | 路由 |
|--------|------|
| 创建/新建订单、按合同生成订单、拆订单 | `sop/order-operations.md` 的“创建订单 / 自动拆单” → 订单专属创建流程 |
...
> 通用写操作流程见 `core/write-engine.md`；订单创建/拆单流程以 `sop/order-operations.md` 的“创建订单 / 自动拆单”为准，完整导出以“完整订单导出”为准。
...
| 全部订单/完整订单明细/全量同步/导出订单 | `sop/order-operations.md` 的“完整订单导出” |
| 创建订单/按合同生成订单/拆订单 | `sop/order-operations.md` 的“创建订单 / 自动拆单” |
