...
```
用户输入
  ├─ 优先级 1：显式模块 + 操作（"查线索"、"创建客户"）→ 直接路由到 `core/cli-spec.md` 对应命令；`线索池` 固定为 `pool/lead`，`公海` 固定为 `pool/account`
...
> **术语硬映射：`线索池` = `pool/lead`；`公海` = `pool/account`。** 该映射在识别池名之前完成，另一模块中的同名池不能覆盖用户明确使用的名词。
