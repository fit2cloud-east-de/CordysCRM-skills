...
- 角色只决定**权限上限**和用户未指定范围时的**默认范围**，不能把用户明确指定的较小范围扩大。经理有权查看部门，不等于每次都查部门。
- 用户明确说“我的 / 我负责的 / 我名下的 / 归我的”，或问“我有哪些 / 我有多少”某类业务记录时，在当前 profile 允许查询该模块的前提下，使用该模块的 `viewId:SELF`；模块没有 SELF 时使用 `owner=当前用户 userId`。不得因用户是经理而改成 `ALL + departmentId`，也不要为这类查询调用部门展开。
- “我的团队 / 我的部门 / 我的下属 / 我们部门 / 团队 / 部门”明确指组织范围，不属于 SELF，按经理的部门及子部门范围处理。
- 范围优先级：权限边界 → 用户明确指定的本人/具体人/团队/公司范围 → 当前角色默认范围。用户未指定范围时，才应用角色默认值。
