...
> - "查一下赛摩智能" / "赛摩智能有没有 MK" / "查查畅联智融的 JS" / "看看赛摩智能公司的 JS" / **直接给一个公司名、手机号或人名** → **查重**（`cordys_ext.sh check`），公司名/人名进 `客户名`、手机号进 `手机`。产品词只用于识别意图，不参与查重判断。查重并行搜索 6 个模块，任一模块查到记录就统一提示“可能存在冲突”。这是**所有角色**的默认查询意图。
