...
> **外部数据流声明**：打卡请求会把 userid、姓名、部门、CRM 跟进记录 ID、客户名、联系人、跟进内容和产品等业务数据发送到 `CHECKIN_API_URL`；若配置 `OPENCLAW_WEBHOOK_URL`，该回调地址也会交给打卡服务。此地址不是 Cordys CRM 域名，部署者应确认接收方与数据用途。
...
| 填写人 | 是 | 姓名（从 Cordys.md / whoami 的 userName 获取） |
| 所在部门 | 是 | 部门（从 Cordys.md / whoami 的 departmentName 获取） |
| 打卡类型 | 是 | `线上拜访` / `线下拜访` / `公司打卡` |
| 用户类型 | 是 | 固定传 `企业微信用户` |
| crmFollowUpId | 拜访打卡必填 | CRM 跟进记录 ID（`cordys_ext.sh follow` 返回的 `data.id`）。公司打卡不传此字段 |
| 拜访公司名称 | 否 | 公司名称（拜访打卡时传） |
| 拜访公司类型 | 否 | 公司类型，必须传中文标签：`线索` / `客户` / `商机`，不要传 API 枚举值 |
| 跟进类型 | 否 | 必须传中文标签：`线索` / `客户` / `商机`，不要传 API 枚举值 |
