...
| 视图名称 | viewId | 启用 | 固定 |
|----------|--------|------|------|
| 东区线索 | `5559096430526464` | 是 | 是 |
| 北区线索 | `5558907451965440` | 是 | 是 |
| 南区线索 | `5558581034450944` | 是 | 是 |
<!-- AUTO-GENERATED-END -->
...
统一执行 `cordys_ext.sh check`（流程见 `SKILL.md` 和 `sop/duplicate-check.md`）：用公司名和手机号并行搜索线索、线索池、客户、公海、商机、联系人 6 个分类；任一分类查到记录就提示可能存在冲突，不按产品细分。不得自行拆成 6 条查询命令。
...
- 并行搜索线索、线索池、客户、公海、商机、联系人
- 6 个分类均无记录 → 未查到相关记录，继续
