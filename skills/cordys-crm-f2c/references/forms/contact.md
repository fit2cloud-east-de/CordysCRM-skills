...
统一执行 `cordys_ext.sh check`（流程见 `SKILL.md` 和 `sop/duplicate-check.md`）：用客户名和联系人手机号并行搜索线索、线索池、客户、公海、商机、联系人 6 个分类；任一分类查到记录就提示可能存在冲突，不按产品细分。不得自行拆成 6 条查询命令。
...
查询和更新也可直接使用 `contact` 别名，例如 `cordys.sh crm update contact '{"id":"...",...}'`；CLI 会自动映射到 `/account/contact/*`。
