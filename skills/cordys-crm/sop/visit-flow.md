...
用户明确说了"线索""商机""客户"时，记录对应的搜索模块；未提则步骤 2 并行搜索三个模块。用户说"创建线索"时，跳过步骤 2，直接走 `core/write-engine.md` 创建线索 → 拿到 clueId → 继续步骤 3。
...
- **0 条匹配** → 问用户是否创建新线索，确认→走 `core/write-engine.md` 创建线索 → 拿到 clueId → 继续步骤 3；拒绝→结束
