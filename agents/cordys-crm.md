...
创建订单时一次命令只传合同 ID 和可选公共默认字段。CLI 按“产品类型 ID + 收入类型中文标签”形成全部订单：共享同一 PRICE 目录的不同产品必须分开，同一业务产品即使来自不同子表或选择器也归入同组；订单名模板不变并逐单顺序创建。全部成功后才回写合同“是否已拆订单=是”，部分成功或状态不明时禁止整批重跑。
